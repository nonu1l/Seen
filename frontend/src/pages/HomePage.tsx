...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen px-3 pb-20 pt-12 sm:px-8 sm:pt-20">
      <div className="mx-auto max-w-4xl">

        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold tracking-tight text-[color:var(--text-primary)] sm:text-3xl">
            seen<span className="text-[color:var(--accent)]">.</span>
          </h1>
          <p className="mt-1.5 text-[13px] text-[color:var(--text-muted)]">
            a private cinema diary
          </p>
        </div>

        {/* Search */}
        <div className="relative mb-4">
          <svg className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[color:var(--text-muted)]"
               fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
          </svg>
          <input type="text" value={query} onChange={e => setQuery(e.target.value)}
                 placeholder={showingSearch ? '搜索中...' : '搜索片名...'}
                 className="search-input" autoFocus />
          {query && (
            <button type="button" onClick={() => setQuery('')}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-[color:var(--text-muted)] hover:text-[color:var(--text-primary)] transition-colors" aria-label="清除">
        ...
        {/* Hint */}
        <p className="mb-5 text-[12px] text-[color:var(--text-muted)]">
        ...
        {isEmpty && (
          <div className="flex flex-col items-center gap-3 py-24 text-center">
            ...
            <p className="max-w-xs text-[13px] text-[color:var(--text-secondary)] leading-relaxed">
        ...
        {merged.marked.length > 0 && (
          <section className="mb-8">
            {showingSearch && <p className="mb-3 text-[12px] font-medium text-[color:var(--text-muted)]">我的标记 · {merged.marked.length}</p>}
        ...
        {showingSearch && merged.unmarked.length > 0 && (
          <section>
            <p className="mb-3 text-[12px] font-medium text-[color:var(--text-muted)]">Bangumi · {merged.unmarked.length}</p>
            <div className="grid gap-1.5 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
