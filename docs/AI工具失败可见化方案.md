...
- 调整 LLM 暴露工具，不改变内部服务优先：
  - `searchWeb` 返回 `WebSearchToolResultDTO`，失败或 0 结果也明确返回原因。
  - `fetchWeb` 返回增强后的结构化结果，保留 `status/error/text`，增加 `ok/hint`。
...
- 调整工具实现边界：
  ...
  - Serper / Tavily provider 把缺少 API Key、HTTP 异常、解析失败和 0 结果传到上层诊断。
...
- 搜索 0 结果：
  - LLM 收到 `ok=false, count=0, error="当前搜索源 returned 0 usable results"`。
- `findWorks` 全流程失败：
...
- 单测：
  - `fetchWeb` 超时时返回 `ok=false` 且包含 error。
...
- 手动验证：
  - 让 Agent 搜索一个无结果关键词，最终回复能说明搜索不到，而不是沉默或编造。
