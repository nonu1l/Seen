...
- AI 工具改用 Spring AI 官方工具注解和提示词模板参数，并用轻量注册器统一控制搜索工具开关、工具调用统计节点和 Anthropic 缓存命中统计。
- 将 AI token 限额、对话上下文、搜索/抓取超时、缓存 TTL 和 Agent 安全阈值等运行参数集中到 `application.yml`。

### 2.2.0-beta5

- 修复 Docker 可执行 jar 中异步请求缓存解析失败的问题，避免打开作品详情时因 Bangumi 详情请求失败返回 404。

### 2.3.0-beta1

- AI 对话主链路改为轻量路由 + 受控子流程：片源、找片推荐、分析和标记操作只暴露各自需要的工具，减少误操作和工具调用失控。
- 对资源搜索，Bangumi搜索使用虚拟线程来执行
- Analysis 子 Agent 支持显式 ReAct 只读分析循环，通过“决策 JSON → 工具观察 → 收束回答”练习多步分析，并保留工具白名单和调用预算限制。
- 增强 SearchPipeline 容错：标题抽取异常时保留搜索结果和网页正文中的片名候选，避免热门推荐因单个 LLM 节点失败而无法展示卡片。
---
...
用户输入
    ...
    ▼
┌────────────────────┐
│ Agent Orchestrator │ ← 读取历史与当前请求，协调本轮 Agent
└────┬───────────────┘
     │
     ▼
┌────────────────────┐
│ Capability Router  │ ← LLM 短路由，结合最近历史判断任务类型
└────┬───────────────┘
     │
     ├─ WatchSource Runner
     ...
     │
     ├─ FindWorks Runner
     │    └─ findAndPresentWorks → SearchPipeline → 生成 PENDING 展示卡片
     │
     ├─ Analysis Runner
     │    └─ searchLocal / getWorkState / readUserMemory / searchWeb(可关闭) / fetchWeb(明确 URL)
     │
     ├─ Action Runner
...
AI 会话每轮生成一个 `requestId`，并写入消息、卡片、record、快照和 Token 用量。当前主链路采用“能力路由 + 受控 Runner”：片源、找片推荐、分析和写库操作只暴露各自需要的工具，避免一个 Agent 同时持有全部能力。标记、修改评分影评和取消标记都由工具直接执行；撤销时按 `ai_work_snapshot` 恢复本轮操作前的完整作品状态。取消标记有误删保护，不支持 AI 直接清空整个片库。
...
用户输入
  ...
  ├─ 2. searchWeb → 按设置页选择 Serper / Tavily；关键词按顺序尝试，命中足够候选后停止
  ├─ 3. fetchWeb → 使用虚拟线程并发抓取搜索结果页面；搜索无结果时由 LLM 选择公开榜单 URL 直访兜底
  ├─ 4. extractTitles → 从页面内容提炼片名
  ├─ 5. searchBangumi → 使用虚拟线程并发匹配 Bangumi 条目
  ├─ 6. 去重：片名和 subjectId 去重
  ├─ 7. validateMatches → 校验候选与用户需求是否匹配
  └─ 8. 聚合最多5条候选；description 模式命中后提前停止，全空时生成失败说明
```

SearchPipeline 本身只负责找出候选作品；在 AI 页面中由 `findAndPresentWorks` 原子工具调用它，并直接创建 `PENDING` 展示卡片，不再依赖模型额外调用 `presentWorks`。
...
用户片源问题
  │
  ├─ 1. WatchSource Runner → 只暴露 searchWatchSources，不暴露通用搜索、找片或写库工具
  ├─ 2. resolveTitle → LLM 从用户问题和补充描述中抽取最可能的影视名称
  ├─ 3. searchWeb → 使用当前设置页选择的 Serper / Tavily 搜索“片名 + 在线观看 + 在哪看”
  ├─ 4. fetchWeb → 并发打开候选 URL，读取页面标题、正文和可访问状态；无法访问的候选直接剔除
  ├─ 5. validateFetchedContent → LLM 根据打开后的页面内容判断候选是否确实匹配目标作品
  ├─ 6. 去重与截断 → 按 URL 去重，限制候选数量和最终返回数量
  └─ 7. 返回可打开的候选观看链接；无可靠候选时返回失败原因，不编造平台或链接
```

在线资源搜索只负责返回候选观看页面，不写入 record，也不生成作品卡片。搜索结果、网页内容和工具返回都只作为资料；最终回复只能展示工具返回的 URL，不补充未返回的平台、版权状态或地区限制说明。
