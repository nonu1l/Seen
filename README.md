...
- 统一 DTO 命名规范，按响应、Bangumi 外部源与 Agent 工具结果收敛前后端接口类型，并合并 Web 抓取与 Bangumi 精简结果的小型重复 DTO。
...
- 设置页额外配置新增深色 / 亮色主题切换，支持本地持久化和动画过渡，并优化亮色主题灰色文字对比度。
...
- 优化自主 Agent 时间表达处理：用户未明确年份时不再擅自补过去年份，“最近 / 当前 / 今年 / 热门”等搜索按今日日期或近期语义组织工具 query。
...
- AI 取消标记加入误删保护，拒绝整库级清空表达，并限制单轮自动取消数量；网页和搜索内容加入轻量提示词注入防护。
...
└────┬───────────────┘
     ...
     └─ readUserMemory / searchWeb(可关闭) / fetchWeb
                         │
                         ▼
