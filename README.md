...
### 2.2.0-beta3
- fetchWeb 增强页面清洗、链接提取和内网地址拦截。
- AI 文本任务思考模式改为在设置页选择默认 / 开启 / 关闭，并按 Base URL 自动适配对应模型服务；搜索开关统一由“搜索源：不使用 / Serper / Tavily”控制。
- 小幅收敛后端 DTO 和旧残留：作品接口请求体改为明确类型，合并请求缓存条目 DTO，清理旧 AI 标记撤销代码。
- AI 新增在线观看地址搜索工具，支持“哪里可以看 / 在哪看 / 片源”等问题，并优先返回更相关的候选链接。
- 在线观看地址会并发打开候选页面，按打开后的页面内容二次筛选，并更宽松保留可能可用的剧集 / 播放页。
- 简化片在线观看候选编号解析，避免成功打开后的非连续编号被误过滤。
- 片源回复只展示工具返回的候选链接，不再补充未验证的平台或版权判断。
- 片源回复要求逐条展示完整 URL，避免只返回站点名或标题。
- 新增文本型 LLM 任务封装，统一处理提示词、思考模式、重试、回调解析和正文清理。
- 片源页面内容校验改用统一的文本型 LLM 任务封装。
- 搜索管道和长期记忆生成也改用统一的文本型 LLM 任务封装。
